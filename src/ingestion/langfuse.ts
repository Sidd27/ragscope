...
function inferKindFromObservation(obs: LangfuseObservation): SpanKind {
  ...
  if (name.includes('retriev') || name.includes('search') || name.includes('query')) return 'RETRIEVER'
...
function observationToSpan(obs: LangfuseObservation, traceId: string): ParsedSpan {
  ...
  const inputText = typeof obs.input === 'string' ? obs.input
    : obs.input ? JSON.stringify(obs.input) : undefined
