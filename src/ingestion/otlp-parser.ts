...
function inferKind(spanName: string, attrs: Record<string, string | number | boolean | null>): SpanKind {
  ...
  if (op === 'chat' || op === 'completion' || op === 'text_generation' || name.includes('llm') || name.includes('chat')) return 'LLM'
  if (name.includes('chain') || name.includes('pipeline') || name.includes('graph')) return 'CHAIN'
  return 'SPAN'
}

function parseDocuments(attrs: Record<string, string | number | boolean | null>, rawAttrs: OtlpAttribute[]): RetrievalDocument[] {
    ...
    for (const ss of rs.scopeSpans ?? []) {
      for (const span of ss.spans ?? []) {
        ...
        const parsed: ParsedSpan = {
          ...
          model: attrs['gen_ai.request.model'] as string | undefined ?? attrs['gen_ai.response.model'] as string | undefined,
