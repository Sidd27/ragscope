  ...
  it('infers LLM kind from operation name', () => {
    const payload: OtlpPayload = {
      resourceSpans: [{
        scopeSpans: [{
          spans: [{
            traceId: 't1', spanId: 's1', name: 'openai.chat', kind: 3,
  ...
  it('falls back to SPAN kind for unknown spans', () => {
    const payload: OtlpPayload = {
      resourceSpans: [{
        scopeSpans: [{
          spans: [{
            traceId: 't1', spanId: 's1', name: 'some.unknown.operation', kind: 1,
            startTimeUnixNano: '1000000000000', endTimeUnixNano: '1000000001000',
          }],
        }],
      }],
    }
  ...
  it('parses indexed gen_ai.retrieval.documents.N.field attributes', () => {
    const payload: OtlpPayload = {
      resourceSpans: [{
        scopeSpans: [{
          spans: [{
            traceId: 't1', spanId: 's1', name: 'qdrant.query', kind: 3,
            startTimeUnixNano: '1000000000000', endTimeUnixNano: '1001000000000',
            attributes: [
              ...
              { key: 'gen_ai.retrieval.documents.0.content', value: { stringValue: 'First chunk' } },
  ...
  it('ignores malformed gen_ai.retrieval.documents JSON and falls back to indexed', () => {
    const payload: OtlpPayload = {
      resourceSpans: [{
        scopeSpans: [{
          spans: [{
            traceId: 't1', spanId: 's1', name: 'retriever', kind: 3,
            startTimeUnixNano: '1000000000000', endTimeUnixNano: '1001000000000',
            attributes: [
              { key: 'gen_ai.retrieval.documents', value: { stringValue: 'not valid json {{' } },
              { key: 'gen_ai.retrieval.documents.0.id', value: { stringValue: 'fallback-doc' } },
  ...
  it('infers EMBEDDING kind', () => {
    const payload: OtlpPayload = {
      resourceSpans: [{
        scopeSpans: [{
          spans: [{
            traceId: 't1', spanId: 's1', name: 'openai.embed', kind: 3,
            startTimeUnixNano: '1000000000000', endTimeUnixNano: '1001000000000',
  ...
  it('infers RERANKER kind', () => {
    const payload: OtlpPayload = {
      resourceSpans: [{
        scopeSpans: [{
          spans: [{
            traceId: 't1', spanId: 's1', name: 'cohere.rerank', kind: 3,
            startTimeUnixNano: '1000000000000', endTimeUnixNano: '1001000000000',
  ...
  it('defaults serviceName to unknown when resource has no service.name', () => {
    const payload: OtlpPayload = {
      resourceSpans: [{
        scopeSpans: [{
          spans: [{
            traceId: 't1', spanId: 's1', name: 'op', kind: 1,
            startTimeUnixNano: '1000000000000', endTimeUnixNano: '1001000000000',
          }],
        }],
      }],
