...
function makeChunk(chunkId: string, rank: number, score = 0.9): RagChunk {
  return {
    id: `id-${chunkId}`, spanId: 's1', traceId: 't1', chunkId,
    content: `content of ${chunkId}`, scoreRaw: score, scoreNormalized: score,
    rankRetrieval: rank, rankReranked: null, scoreReranked: null, tokenCount: 5,
    vectorStore: null, inContext: false, contextPosition: null,
    overlapWithNext: 0, scoreMissing: false,
  }
}

function makeRerankerSpan(docs: Array<{ id: string; score: number }>): ParsedSpan {
  return {
    traceId: 't1', spanId: 'reranker-span', name: 'cohere.rerank',
    kind: 'RERANKER', startTimeMs: 200, endTimeMs: 350, latencyMs: 150,
    rawAttributes: '[]', documents: docs,
  }
}

describe('applyRerankerResults', () => {
  const chunks = [
    makeChunk('doc-a', 1, 0.80),
    makeChunk('doc-b', 2, 0.75),
    makeChunk('doc-c', 3, 0.60),
  ]
  ...
  it('includes scoreDelta in diffs', () => {
    ...
    expect(diff.scoreDelta).toBeCloseTo(0.95 - 0.80)
  })
