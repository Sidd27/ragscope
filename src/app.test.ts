...
function makeTrace(id: string): RagTrace {
  return { id, serviceName: 'svc', query: 'q', source: 'manual', totalLatencyMs: 100, spanCount: 1, chunkCount: 0, createdAt: Date.now() }
}
...
const otlpTrace = {
  resourceSpans: [{
    ...
    scopeSpans: [{
      spans: [{
        traceId: 'app-test-trace-001', spanId: 'span-1',
        name: 'qdrant.query', kind: 3,
