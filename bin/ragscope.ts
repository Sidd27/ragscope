...
function printAudit(result: AuditResult): void {
  ...
  if (verbose) {
    ...
    const subs = result.subscores.map(s => {
      const c = s.symbol === '✓' ? pc.green : s.symbol === '~' ? pc.yellow : pc.red
...
function printSession(): void {
  ...
  const trend = sessionScores.length < 2 ? '' :
    last > avg ? pc.green(' ↑') : last < avg ? pc.red(' ↓') : pc.dim(' →')
...
async function handleTrace(traceId: string): Promise<void> {
  ...
  if (!result) return
  const audit = scoreTrace(result.trace.serviceName, result.trace.query, result.spans, result.chunks)
